...
function applyFilter(rows, filter) {
  ...
  return rows.filter(r => (r.email||'').toLowerCase().includes(f) || (r.room||'').toLowerCase().includes(f));
}
...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    function refresh() {
      ...
      document.getElementById('avgTime').textContent = s.avg == null ? '-' : formatHMS(s.avg);
      document.getElementById('peakTime').textContent = s.peak == null ? '-' : formatHMS(s.peak);
    }
    ...
    console.error(err);
    document.querySelector('#mainContent').innerHTML = '<p>Error loading analytics data.</p>';
  }
});
